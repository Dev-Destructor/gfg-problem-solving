...
class Solution{
    public static Node buildTree(ArrayList<Integer> nodes, int root) {
        if (root >= nodes.size()) {
            return null;
        }
        
        Node newNode = new Node(nodes.get(root));
        newNode.left = buildTree(nodes, (2 * root) + 1);
        newNode.right = buildTree(nodes, (2 * root) + 2);
    ...
    public static void createTree(Node root0, ArrayList<Integer> v ){
        ...
        root0.left = buildTree(v, 1);
        root0.right = buildTree(v, 2);
    }
}